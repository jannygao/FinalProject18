...
score = 0 #just a random variable to help it keep track but i also am too lazy to implement a scoring system
blockIndex = random.randint(0,19) #establishes the numbers to randomly select as the index to randomly choose a block
colorIndex = blockIndex + 1 #keeps the color assigned to each block because of indexes
...
def draw():
    ...
    display(grid, 0, 0, colorIndex) #shows the grid and the change
    
    display(blockShapes[blockIndex], mouseX-boxSize/2, mouseY-boxSize/2, colors[colorIndex]) #shows the block being moved around
    ...
    for i in range(len(piece)):
        ...
        for j in range(len(piece[0])):
            if gridX+j >= gridSize:
                ...
                canPlace = False
            elif grid[gridY+i][gridX+j] > 0 and piece[i][j] > 0:
                canPlace = False
    if canPlace == True:
        for i in range(len(piece)):
            for j in range(len(piece[0])):
                if piece[i][j] > 0:
                   #adding pieces to the grid if they fit
                     grid[gridY+i][gridX+j] = piece[i][j]
